...
Browser-based viewer for structural, functional, and diffusion MRI atlases.
Built on [NiiVue](https://niivue.com).
...
* **fMRI** — time-course playback on the AAL3 brain with synchronized slices.
* **DTI** — an FA/scalar-map viewer with JHU ICBM-DTI-81 and SMATT
  white-matter tract overlays.
...
* No internet required — NiiVue, Three.js, TensorFlow.js, brainchop, and the
  required structural/diffusion atlas volumes are vendored in the repository.
  The local server verifies every bundled NIfTI by SHA-256 at startup and only
  downloads an official replacement when a file is missing or corrupted.
  Public CDNs are used only as a fallback if a local library copy is missing.
...
That's it. The server starts on `http://localhost:8765`, verifies the bundled
atlas assets, and auto-opens your browser.
...
## DTI tab (diffusion scalar maps and tract atlases)

The **DTI** tab opens a four-panel NiiVue workspace (axial, sagittal, coronal,
and 3-D) on the bundled 2-mm JHU ICBM fractional-anisotropy template.

* **Two bundled DTI atlases** — choose **JHU ICBM-DTI-81** (50 canonical
  white-matter labels) or **SMATT** (60 left/right sensorimotor tract
  combinations). Both label maps and their official FSL metadata ship with the
  static site.
* **Tract controls** — search, filter by hemisphere, show/hide/invert labels,
  or double-click a row to isolate one tract. Changes update slices and 3-D
  rendering together.
* **Bring your own scalar map** — load FA, MD, AD, RD, or another 3-D NIfTI.
  Files stay in the browser. Atlas labels are meaningful only when the subject
  map has been registered to the selected atlas space.
* **Publication capture** — switch between the combined view, individual slice
  planes, and 3-D rendering, then save directly or add the view to the shared
  figure collection.
...
Capture views from any visualization tab with the floating **Collection** panel,
then choose **Open arranger** to build a publication-ready multi-panel figure.

* **Reliable grid layouts** — use Auto or an explicit 1–4 columns, with editable
  outer margins and panel gaps. Images are contain-fitted without stretching or
  clipping, including tall one- and two-column layouts.
* **Page controls** — choose Letter, A4, widescreen, square, or a custom pixel
  size, plus white, dark, transparent, or custom-colour backgrounds.
* **Editable typography** — add headings and free text; customize font, size,
  weight, italics, alignment, line height, colour, fill, opacity, and padding.
  Image labels have independent text, position, font, colour, and fill controls.
* **Freeform finishing** — drag, resize, layer, duplicate, nudge, or delete any
  panel. The page preview and exported PNG use the same geometry and styles.
...
*Note:* this is the gray-matter atlas used by the Figure tab. The DTI tab's
separate JHU entry is the white-matter ICBM-DTI-81 atlas below.

### JHU ICBM-DTI-81 — White-Matter Labels (DTI tab)

> Mori S, Oishi K, Jiang H, Jiang L, Li X, Akhter K, Hua K, Faria AV,
> Mahmood A, Woods R, Toga AW, Pike GB, Neto PR, Evans A, Zhang J, Huang H,
> Miller MI, van Zijl P, Mazziotta J. (2008).
> *Stereotaxic white matter atlas based on diffusion tensor imaging in an
> ICBM template.* NeuroImage 40(2):570–582.
> doi:[10.1016/j.neuroimage.2007.12.035](https://doi.org/10.1016/j.neuroimage.2007.12.035)

### SMATT — Human Sensorimotor Tracts (DTI tab)

> Archer DB, Vaillancourt DE, Coombes SA. (2018).
> *A template and probabilistic atlas of the human sensorimotor tracts using
> diffusion MRI.* Cerebral Cortex 28(5):1685–1699.
> doi:[10.1093/cercor/bhx066](https://doi.org/10.1093/cercor/bhx066)
...
**Advanced figure arrangement** — replaced the fixed-aspect Arrange modal with
a dedicated top-level workspace, fixed one- and two-column layouts, and added
page, spacing, label, typography, positioning, layering, and PNG export controls.

### 2026-07-13

**DTI visualization tab** — added a lazy, single-context diffusion viewer with
combined slice/3-D rendering, subject scalar-map upload, tract search/isolation,
display controls, PNG/collection capture, and two bundled white-matter atlases:
JHU ICBM-DTI-81 and SMATT. Atlas metadata, label parsing, filtering, and LUT
generation live in a reusable/testable DTI core instead of the tab controller.
